  ...
  return (
    <Sidebar className={isCollapsed ? "w-14" : "w-64"} collapsible="icon">
        ...
        <div className="flex items-center gap-2 px-4 py-3">
          {!isCollapsed && (
            <>
              ...
              <div>
                <h2 className="text-sm font-semibold text-text1">WeDo 2.0</h2>
                <p className="text-xs text-text4">{t("app.subtitle")}</p>
              </div>
            </>
          )}
          {isCollapsed && (
            <div className="w-8 h-8 mx-auto rounded-lg bg-gradient-to-br from-[hsl(var(--grad-hero-from))] to-[hsl(var(--grad-hero-to))] flex items-center justify-center">
              <span className="text-sm font-bold text-bg">W</span>
            </div>
          )}
        </div>
      ...
      <SidebarContent>
        <SidebarGroup>
          {!isCollapsed && (
            <SidebarGroupLabel className="text-text3 px-4">
              {t("menu.main")}
            </SidebarGroupLabel>
          )}
          <SidebarGroupContent>
                ...
                <SidebarMenuItem key={item.to}>
                  <SidebarMenuButton asChild>
                    <NavLink
                      ...
                      className="flex items-center gap-3 px-4 py-2.5 text-text2 hover:bg-surface3 hover:text-text1 transition-colors"
                      activeClassName="bg-gradient-to-r from-[hsl(var(--grad-hero-from))]/10 to-[hsl(var(--grad-hero-to))]/10 text-text1 font-medium border-l-2 border-[hsl(var(--grad-hero-from))]"
                    >
