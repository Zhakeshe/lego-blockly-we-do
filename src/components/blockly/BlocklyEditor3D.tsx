...
const BlocklyEditor3D: React.FC<BlocklyEditor3DProps> = ({ onSave, initialWorkspace }) => {
  ...
  useEffect(() => {
    ...
    // 1️⃣ Custom blocks and generators
    defineCustomBlocks();
    ...
    // 3️⃣ Load initial workspace if provided
    if (initialWorkspace) {
      try {
    ...
    // 4️⃣ Create code executor
    executorRef.current = new CodeExecutor(updateMotorPower, getSensorValue);

    // 5️⃣ Listen to workspace changes
    ws.addChangeListener(() => {
      const code = Blockly.JavaScript.workspaceToCode(ws); // ✅ must use Blockly.JavaScript
      setGeneratedCode(code);
    });

    return () => ws.dispose();
  }, [initialWorkspace, updateMotorPower, getSensorValue]);
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      toast({ title: 'Execution Error', description: error.message || 'Failed to execute code', variant: 'destructive' });
    } finally {
  ...
  const handleStop = () => {
    if (executorRef.current) {
      ...
      setIsExecuting(false);
      toast({ title: 'Stopped', description: 'Code execution stopped' });
    }
  };
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="p-4 bg-background border-b flex items-center gap-2">
        <Button onClick={handleRun} disabled={isExecuting || !generatedCode} className="gap-2">
          <Play className="h-4 w-4" /> Run
        </Button>
        <Button onClick={handleStop} disabled={!isExecuting} variant="destructive" className="gap-2">
          <Square className="h-4 w-4" /> Stop
        </Button>
        <Button onClick={() => setShowCode(!showCode)} variant="outline" className="gap-2">
          <Code className="h-4 w-4" /> {showCode ? 'Hide Code' : 'Show Code'}
        </Button>
        <Button onClick={handleSave} variant="outline" className="gap-2 ml-auto">
      ...
      <div className="flex-1 flex">
        <div ref={blocklyDiv} className={`${showCode ? 'w-1/2' : 'w-full'} h-full`} />
        {showCode && (
